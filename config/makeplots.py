...
histogram_xaxes = ["CSVv2 discriminator", "particle ID", "Track p [GeV/c]", "#eta", "#Delta #eta(track,jet axis)", "#Delta R(track,jet axis)", "3D IP of tracks", "3D IP significance of tracks", "2D IP of tracks", "2D IP significance of tracks", "L_{d} [cm]", "#chi^2_{track}", "Number of hits", "Number of pixel hits"," "," "," "," ","d [cm]"," "," "," "," ", "dist_T(PV,SV) [cm]", "dist_T/#sigma_{dist_T}(PV,SV)", "dist(PV,SV) [cm]", "SV 3D flight distance significance", "#Delta#phi", "Number of SV", "N_{tracks}", "inv. mass [GeV/c^2]", "#chi^2_{SV}", "N_{dof}", "#chi^2/N_{dof}(SV)", "category", "inv. mass [GeV/c^{2}]", "E_{SV}/E_{tot}", "IP/#sigma_{IP}","Number of selected tracks in the jets","pT [GeV/c]"]
...
for histogram_name_index in range(len(histogram_names)):
  ...
  #stacked_mc[histogram_name_index].GetXaxis().SetTitle(histogram_xaxes[histogram_name_index])
    ...
    text7.Draw();
    text5 = TLatex(0.98,0.95125,"#sqrt{s} = 13 TeV, 50 ns")
    text5.SetNDC();
