...
bool subjet_plots     = false;
bool btaginfo_plots   = false;
bool additional_plots = false;
...
Hists_sub::Hists_sub(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("tau21", "tau21 (ungroomed)", 50, 0., 1.);
  book<TH1F>("tau3", "tau3 (ungroomed)", 50, 0., 1.);
  book<TH1F>("tau2", "tau2 (ungroomed)", 50, 0., 1.);
  book<TH1F>("tau1", "tau1 (ungroomed)", 50, 0., 1.);

  book<TH1F>("tau32_groomed", "#tau_{3}/#tau_{2} (groomed)", 50, 0., 1.);
  book<TH1F>("tau21_groomed", "tau21 (groomed)", 50, 0., 1.);
  book<TH1F>("tau3_groomed", "tau3 (groomed)", 50, 0., 1.);
  book<TH1F>("tau2_groomed", "tau2 (groomed)", 50, 0., 1.);
  book<TH1F>("tau1_groomed", "tau1 (groomed)", 50, 0., 1.);
  ...
  book<TH1F>("topDeepCSV", "CSV discriminator", 100, 0., 1.);  
...
void Hists_sub::fill(const Event & event){
  ...
  JetId my_checkbtag = DeepCSVBTag( DeepCSVBTag::WP_LOOSE );
  ...
  for(unsigned int n=0;n<topjets->size();n++){
    ...
    hist("softdropmass")->Fill(topjet.softdropmass(),weight);
    ...
    hist("tau1")->Fill(topjet.tau1(), weight);

    hist("tau32_groomed")->Fill(topjet.tau3_groomed()/topjet.tau2_groomed(), weight);
    hist("tau21_groomed")->Fill(topjet.tau2_groomed()/topjet.tau1_groomed(), weight);
    hist("tau3_groomed")->Fill(topjet.tau3_groomed(), weight);
    hist("tau2_groomed")->Fill(topjet.tau2_groomed(), weight);
    hist("tau1_groomed")->Fill(topjet.tau1_groomed(), weight);
    ...
    hist("topDeepCSV")->Fill(topjet.btag_DeepCSV(),weight);
    ...
    JetId checkbtag=DeepCSVBTag(DeepCSVBTag::WP_LOOSE);
    
    if(subjets.size() == 3 && subjet_plots)
      {
